...
> superficie utilitaria: índice de 41 materias filtrable). Evaluación A + B (`detect.mjs` sin
> hallazgos). Sin navegador: sin overlay. Baseline oficial que E07 re-mide.
...
| 8 | Estético y minimalista | 3 | Cubiertas coloreadas por año dan identidad; grilla larga homogénea pide ritmo editorial. |
...
cobalto profundo) es un recurso con voz y la pieza utilitaria más lograda en identidad. El
riesgo es **usabilidad de índice largo**: filtrar 41 ítems solo por año, sin búsqueda ni
feedback de estado, no escala bien. E05 debe volver el filtrado legible y tranquilo sin tocar el
motor data-loader ni el esquema JSON.
...
para 41 materias. Oportunidad mayor para E05: cerrar el bucle de feedback del filtro, sumar
búsqueda por nombre y dar a la grilla un ritmo editorial (no solo más tarjetas iguales).

## What's Working

- **Color por año** como sistema de reconocimiento visual — identidad utilitaria a conservar.
...
- **[P1] Filtro por año sin feedback de estado (visibilidad).** *Por qué importa:* al ocultar
  ...
  el cambio es silencioso. *Fix:* región `aria-live` con conteo de resultados y estado vacío
  explícito si un año queda sin nada. *Resuelve:* **E05** → `/impeccable harden`.
- **[P1] 41 materias sin búsqueda textual (reconocer vs recordar / eficiencia).** *Fix:*
  campo de búsqueda/filtro por nombre además del año. *Resuelve:* **E05** → `/impeccable harden`
  (+ `clarify` para el copy del campo).
- **[P2] Estado del filtro no reflejado en URL.** Impide compartir "apuntes de 3.º".
  *Resuelve:* **E05** → `/impeccable harden`.
...
- El `<h2>` "Materias del Plan 2023" es `sr-only`: bien para lectores, pero verificar que la
  jerarquía visual no quede huérfana al elevar (E05).
