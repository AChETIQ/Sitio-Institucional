...
- **Trazo con presencia.** Las ramas y el borde del aro usan `--tl-tube` **3 px**; el tronco central sube a `--tl-trunk` **4 px** (un punto más, para leerse como eje estructural). Quedan por encima de los filetes de texto (`--rule` 1 px / `--rule-ink` 1.5 px) sin volver al «tubo» de 10 px. La rama usa `vector-effect: non-scaling-stroke` para conservar el grosor a cualquier escala.
- **Anclaje al título.** El título de la sección cede su separación inferior (`margin-bottom: --space-4`) y el tronco arranca en un **nodo de origen** (`.timeline-root::after`, disco cobalto) situado justo debajo: la línea «cae» desde el título. La geometría (`--tl-*`, incl. `--tl-rail` = posición del tronco) vive en el **contenedor de sección** para que la raíz (`.timeline-root`) y el eje del `<ol>` compartan medidas y posición (centrado en desktop, carril de 32 px en < 880 px).
...
- **Entrada ghost (`--ghost`):** rama punteada (2 px) y aro dashed en `--color-border` con glifo de tres puntos; título "Próximamente" en `Fraunces` itálica `--color-text-disabled`.
